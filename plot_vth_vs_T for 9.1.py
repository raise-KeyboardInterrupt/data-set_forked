...
from threshold_utils import find_threshold_at_fixed_current
...
def main():
    ...
    for fname in FILES:
        ...
        V_th, I_th, V_th_sigma, _ = find_threshold_at_fixed_current(
            ...
            make_plot=False,   # don't redraw I-V curves
        )
    ...
    ax.set_title("Threshold voltage vs Temperature for 9.1 V diode (fixed I)")
    ax.set_xlabel("Temperature T (K)")
    ax.set_ylabel(r"Threshold voltage $V_{\mathrm{th}}$ (V)")
    ax.grid(True, ls=":")

    # --- Optional: weighted linear fit V_th(T) ---
    # use V_err as y-uncertainties
    sigma = V_err.copy()
    # if any sigma <= 0, replace with the smallest nonzero sigma
    if np.any(sigma <= 0):
        nz = sigma[sigma > 0]
        if nz.size > 0:
    ...
    m, c = np.linalg.lstsq(Aw, yw, rcond=None)[0]  # V = m T + c
